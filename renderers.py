...
class MjRenderer:
    ...
    def read_pixels_from_camera(self, camera, frame_type):
        self.image_renderer.update_scene(self.mj_data, camera=camera)
        if frame_type == 'depth':
            self.image_renderer.enable_depth_rendering()
            org = self.image_renderer.render()
            image = org[:, :]
            ...
            self.image_renderer.enable_segmentation_rendering()
            org = self.image_renderer.render()
            geom_ids = org[:, :, 0]
            ...
            image = (255 * geom_ids).astype(np.uint8)

        else:
            raise ValueError('Not supported frame type.')
